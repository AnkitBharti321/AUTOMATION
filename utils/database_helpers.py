...
def check_all_shards(sql_query, attempts=10, return_list=False, multiple_columns=False):
...
def get_job_completed_percentage(job_id):
    percent = check_shard_zero(f"SELECT percent_done FROM job_requests WHERE id = {job_id}")
    return percent
...
def execute_hcm_service_db_query(sql_query, attempts=10):
    my_db = db_base.establish_hcm_service_database()
    cursor = my_db.cursor()
    cursor.execute(sql_query)
    result = cursor.fetchone()
    while attempts > 0 and not result:
        cursor.execute(sql_query)
        result = cursor.fetchone()
        time.sleep(1)
        attempts -= 1
...
def execute_hcm_service_db_query_fetchall(sql_query, attempts=10):
    my_db = db_base.establish_hcm_service_database()
    cursor = my_db.cursor()
    cursor.execute(sql_query)
